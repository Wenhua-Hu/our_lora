...
class AdamW(Optimizer):
    ...
    def step(self, closure: Callable = None):
        ...
        """
        # base_model.model.model.layers.0.self_attn.q_proj.base_layer  -4, -2,-1
        # base_model.model.model.layers.0.self_attn.q_proj.lora_A.default  -5,-3,-2
        ...
        for group in self.param_groups:
            for i, p in enumerate(group["params"]):
                ...
                # GaLore Projection
                if "rank" in group:
                    galore_params_name = self.galore_params_names[i]
                    component_names = galore_params_name.split(".")
                    block, nn_name, key_name = int(component_names[4]), component_names[6], component_names[7]
                    self.layers[block][nn_name][key_name] = p  # or p ?

                            
            for block, block_data in self.layers.items():
                for nn_name, nn_data in block_data.items():
                    p = nn_data["base_layer"]
